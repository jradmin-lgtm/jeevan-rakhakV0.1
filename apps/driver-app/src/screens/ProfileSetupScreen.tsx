...
type Props = {
  ...
  /** Called after server confirms the row — driver app then routes into KYC. */
  onSetupComplete: (profile: any) => void;
  onBack: () => void;
};

/**
 * Driver-app first-sign-in profile capture. Same shape as user app's screen
 * but the Continue button copy reads "Continue to KYC" so the driver knows
 * what's next. Phone + name are the only fields here; vehicle/RC/insurance
 * are captured on the KYC screen that follows.
 */
export function ProfileSetupScreen({ idToken, google, onSetupComplete, onBack }: Props) {
  ...
  // CR#5A: reopen the Google account picker and re-run the start handshake.
  const switchAccount = async () => {
    setErr(null);
    setSwitching(true);
    try {
      const res = await switchGoogleAccount();
      const r = await authApi.googleStart(res.idToken);
      if (r.needsProfile) {
  ...
  const submit = async () => {
    ...
    setErr(null);
    try {
      const r = await authApi.googleComplete({ idToken: tok, phone, name: name.trim() });
      await setToken(r.accessToken);
      onSetupComplete(r.profile);
    } catch (e: any) {
      const msg = String(e?.message ?? "");
      if (msg.includes("phone_already_used")) setErr(t("auth.google.error_phone_used"));
      else if (msg.includes("email_already_used")) setErr(t("auth.google.error_email_used"));
      else setErr(e?.message ?? t("auth.google.error_generic"));
    } finally {
      setBusy(false);
    }
  };
