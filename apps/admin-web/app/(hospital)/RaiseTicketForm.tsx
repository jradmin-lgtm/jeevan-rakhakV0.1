...
/**
 * Shared "Raise a ticket" form (v1.2.1, CR#3). Used by:
 *  - the Support page (opened as GENERAL),
...
export function RaiseTicketForm({
  ...
  const submit = async () => {
    setError(null);
    if (tooShort) {
      setError("Please describe the concern in at least 5 characters.");
      return;
    }
    setSubmitting(true);
    try {
      const body: Record<string, unknown> = { subjectType: subject, message: trimmed };
  ...
  return (
    <div className="card" style={{ display: "grid", gap: 12, borderLeft: "4px solid var(--accent)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        <h3 style={{ margin: 0 }}>Raise a concern / feedback</h3>
        {contextLabel ? (
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          placeholder="Describe the issue or feedback (min 5 characters)…"
